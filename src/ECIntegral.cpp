 ...
 * Integrate().
 */
struct FGMemoEntry { double F; double G; double whit; };
...
struct ECWorkspace {
  ...
  ECWorkspace() : w(gsl_integration_workspace_alloc(1000)) {}
  ~ECWorkspace() { if(w) gsl_integration_workspace_free(w); }
};
 ...
 * ------------------------------------------------------------------------ */
struct RateGridKey {
  int z1, z2; double redmass; int lf; double bindingEnergy; double chanrad;
...
const RateGridEntry& getRateGrid(WhitFunc* wf, int lf, double bindingEnergy, double chanrad) {
  ...
  RateGridEntry e; e.ok=false; e.refOut=0.0;
  ...
  if(b > 1.0e-6 && std::isfinite(b)) {
    ...
    if(ws) {
      ...
      double refOut; wf->Scaled(lf, chanrad, bindingEnergy, refOut);
      e.refOut = refOut;
      e.x.resize(kRateRadialPoints); e.W.resize(kRateRadialPoints);
      ...
      for(int k=0;k<kRateRadialPoints;++k) {
        ...
        double sk; double mk = wf->Scaled(lf, xk[k], bindingEnergy, sk);
...
void ECIntegral::Integrate(double chanrad) {
  ...
  if(params_.pairEnergy<0.0) {
    ...
    const RateGridEntry* grid = rateMode ?
      &getRateGrid(params_.whitFunc.get(), params_.lfValue, params_.bindingEnergy, chanrad)
      : nullptr;
    if(grid && grid->ok) {
      ...
      FW_=(double)fw; GW_=(double)gw;
    } else {
      gsl_function FW; FW.function = &FWIntegrand; FW.params= &params_;
      gsl_function GW; GW.function = &GWIntegrand; GW.params= &params_;
...
complex ECIntegral::CalculateDirect(int theInitialLValue, int theFinalLValue, 
  ...
  if(outEnergy>0.0) {
    ...
    if(params_.log10RefOut != 0.0) {
      if(params_.log10RefOut < -290.0) overlapIntegral = complex(0.0,0.0);
      else if(params_.log10RefOut > 290.0) overlapIntegral *= pow(10.0,290.0);
      else overlapIntegral *= pow(10.0,params_.log10RefOut);
    }
  } else {
