...
Equation::Equation(std::string equation, int numParams, const Config &configure) :
  ...
  try {
    ...
    while (position < infixEquation_.length()) {
      ...
      if (tempPair.first == NUMBER || tempPair.first == VARIABLE ||
          tempPair.first == PARAMETER || tempPair.first == FUNCTION)
        expecting = OPERATOR | RIGHTPAR;
      else if (tempPair.first == OPERATOR || tempPair.first == LEFTPAR)
        expecting = FUNCTION | NUMBER | PARAMETER | VARIABLE | LEFTPAR;
      ...
      if (tempPair.first == NUMBER || tempPair.first == PARAMETER || tempPair.first == VARIABLE || tempPair.first == FUNCTION) {
        ...
        } else if (stack[stack.size() - 1].first == LEFTPAR)
          stack.pop_back();
      }
    }
...
void Equation::SetParameter(unsigned int index, double value, const Config &configure) {
  if (index < parameters_.size()) {
    ...
    configure.outStream << "Error: Parameter index " << index << " greater than vector size." << std::endl;
}
...
Equation::TokenPair Equation::GetToken(unsigned int &position, const Config &configure) {
  try {
    ...
    if (IsDigit(infixEquation_[position]) || infixEquation_[position] == '.') {
      tempType = NUMBER;
      while ((IsDigit(infixEquation_[position]) || infixEquation_[position] == '.' || infixEquation_[position] == 'e' || infixEquation_[position] == 'E' || (infixEquation_[position] == '-' && (infixEquation_[position - 1] == 'e' || infixEquation_[position - 1] == 'E')) || (infixEquation_[position] == '+' && (infixEquation_[position - 1] == 'e' || infixEquation_[position - 1] == 'E'))) && position < infixEquation_.length()) {
    ...
    else if (infixEquation_[position] == '-' && (position == 0 || IsOperator(infixEquation_[position - 1]) || infixEquation_[position - 1] == '(')) {
      tempType = FUNCTION;
      position++;
      tempString = "neg";
    }
    ...
    // Unrecognized Tokens
      ...
      throw SyntaxError(infixEquation_, 0, position);
