  ...
  if(isBetaDelayed) {
    ...
    if(compound()->GetPair(exitPairNum)->GetPType() == 10) {
      ...
      for(int i = 1; i <= nTerms; i++) {
        ...
        complex prod = element(t->kGroup1, t->path1, t->isEC1)
                     * conj(element(t->kGroup2, t->path2, t->isEC2));
      ...
      for(int i = 1; i <= nTerms; i++) {
        ...
        if(t->isEC1) ecBar[t->kGroup1-1][t->path1-1] += fitBar * d1;
        else          tBar[t->kGroup1-1][t->path1-1] += fitBar * d1;
        if(t->isEC2) ecBar[t->kGroup2-1][t->path2-1] += fitBar * d2;
        else          tBar[t->kGroup2-1][t->path2-1] += fitBar * d2;
      }
