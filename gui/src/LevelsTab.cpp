...
void LevelsTab::addLevel() {
  ...
  if (aDialog.exec()) {
    ...
    if (aDialog.piValueCombo->currentIndex() == 0)
      newLevel.piValue = -1;
    else
      newLevel.piValue = 1;
...
void LevelsTab::editLevel() {
  ...
  if (piValue == -1)
    aDialog.piValueCombo->setCurrentIndex(0);
  else
    aDialog.piValueCombo->setCurrentIndex(1);
  ...
  if (indexes.isEmpty()) {
    ...
    if (channel.radType == 'P') {
      ...
      if (channel.radType == 'G')
        stm << QString("Channel is Gamow-Teller beta decay") << Qt::endl
            << Qt::endl;
      else
        stm << QString("Channel is Fermi beta decay") << Qt::endl
            << Qt::endl;
      ...
      if (((channel.radType == 'E' && channel.lValue == 1) &&
        ...
        stm << "Capture is internal only" << Qt::endl;
      stm << Qt::endl;
    }
    ...
    if (level.energy < (pair.seperationEnergy + pair.excitationEnergy) && pair.pairType == 0)
    ...
    double reducedMass = (pair.lightM + pair.heavyM != 0.)
        ? pair.lightM * pair.heavyM / (pair.lightM + pair.heavyM)
        : 0.;
    double eCm = level.energy - pair.seperationEnergy - pair.excitationEnergy;
    wignerApplicable_ = (channel.radType == 'P') && (eCm > 0.) && (pair.channelRadius > 0.) && (reducedMass > 0.);
    wignerZ1_ = pair.lightZ;
    wignerZ2_ = pair.heavyZ;
    wignerRedMass_ = reducedMass;
    wignerRadius_ = pair.channelRadius;
    wignerEcm_ = eCm;
    wignerL_ = channel.lValue;
...
void LevelsTab::calculateWignerLimit() {
  ...
  if (status != 0 || denom <= 0.) {
    channelDetails->wignerLimitText->setText(tr("error"));
    return;
  }
...
bool LevelsTab::readNuclearFile(QTextStream &inStream) {
  ...
  while (!inStream.atEnd() && line.trimmed() != QString("</levels>")) {
    ...
    if (!inStream.atEnd() && line.trimmed() != QString("</levels>")) {
      QTextStream in(&line);
      in >> levelJ >> levelPi >> levelEnergy >> levelFix >> aa >> ir >> channelS >> channelL >> levelId >> levelYN >> channelFix >> channelReducedWidth >> lightJ >> lightPi >> heavyJ >> heavyPi >> excitationEnergy >> lightM >> heavyM >> lightZ >> heavyZ >> seperationEnergyIn >> seperationEnergyOut >> dummyInt >> dummyInt >> dummyDouble >> pairType >> channelRadius >> lightG >> heavyG;
      ...
      if (pair.pairType == 10 && which == 0)
        ...
        if (it != elementMap.end()) {
          if (tempM == 1) {
            ...
            return QString("$^{%1}$%2").arg(tempM).arg(it->second);
        } else
          return "?";
      }
    }
