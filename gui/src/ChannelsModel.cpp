...
bool ChannelsModel::setData(const QModelIndex &index, const QVariant &value, int role) {
  if (index.isValid()) {
    if (role == Qt::EditRole) {
      ...
      if (index.column() == 0)
        tempData.isFixed = value.toInt();
      else if (index.column() == 1)
        tempData.levelIndex = value.toInt();
      else if (index.column() == 2)
        tempData.pairIndex = value.toInt();
      else if (index.column() == 3)
        tempData.sValue = value.toDouble();
      else if (index.column() == 4)
        tempData.lValue = value.toInt();
      else if (index.column() == 5)
        tempData.radType = value.toChar();
      else if (index.column() == 6)
        tempData.reducedWidth = value.toDouble();
      else if (index.column() == 7)
        tempData.gammaIsRWA = value.toInt();
      else
        return false;
...
QString ChannelsModel::getSpinLabel(const ChannelsData &channel) const {
  if (((int)(channel.sValue * 2)) % 2 != 0 && channel.sValue != 0.)
    return QString("%1/2").arg((int)(channel.sValue * 2));
  else
    return QString("%1").arg(channel.sValue);
}
