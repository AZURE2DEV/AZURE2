...
QVariant LevelsModel::data(const QModelIndex &index, int role) const {
  ...
  if (role == Qt::DisplayRole) {
    ...
    if (index.column() == 2)
      return getSpinLabel(level);
    else if (index.column() == 3)
      return level.piValue;
  ...
  if (index.isValid()) {
    if (role == Qt::EditRole) {
      ...
      if (index.column() == 0)
        ...
        tempData.jValue = value.toDouble();
      else if (index.column() == 3)
        tempData.piValue = value.toInt();
      else if (index.column() == 4)
        tempData.energy = value.toDouble();
      else
        return false;
...
QString LevelsModel::getSpinLabel(const LevelsData &level) const {
  QString tempSpin;
  if (((int)(level.jValue * 2)) % 2 != 0 && level.jValue != 0.)
    tempSpin = QString("%1/2").arg((int)(level.jValue * 2));
  else
    tempSpin = QString("%1").arg(level.jValue);
  if (level.piValue == -1)
