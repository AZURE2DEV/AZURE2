...
void CNuc::SortPathways(const Config& configure) {
  ...
  for(int aa=1;aa<=this->NumPairs();aa++) {
	...
	for(int l = 0; l < 2; l++) {
      ...
      } else if(this->GetPair(ir)->GetPType()==0&&aa==ir) { //nuclear particles this->GetPair(ir)->IsUPOS()==false this->GetPair(ir)->GetPType()==0&&aa==ir
	for(double s=fabs(this->GetPair(aa)->GetJ(1)-this->GetPair(aa)->GetJ(2));
      ...
      } else if(this->GetPair(ir)->GetPType()==0&&aa!=ir){//adding additional Groups for Unobserved Primary, Observed Secondary &&this->GetPair(ir)->IsUPOS()==true &&aa!=ir
        for(double s=fabs(this->GetPair(aa)->GetJ(1)-this->GetPair(aa)->GetJ(2));
	    s<=(this->GetPair(aa)->GetJ(1)+this->GetPair(aa)->GetJ(2));s+=1.) {
	  for(double sp=fabs(this->GetPair(ir)->GetJ(1)-this->GetPair(ir)->GetJ(2));
	      sp<=(this->GetPair(ir)->GetJ(1)+this->GetPair(ir)->GetJ(2));sp+=1.) {
...
void CNuc::CalcBoundaryConditions(const Config& configure){
  ...
  std::ifstream bcoption;
  bcoption.open ("bcflag.dat");
  int bcflag=0;
  double bcvalue=0.0;
  if(bcoption){
      ...
      for(int k=1;k<=theDecay->NumKGroups();k++) {
		...
		if(this->GetPair(theDecay->GetPairNum())->GetPType()==0) {
		  ...
		  z1z2=pow(-1.,1.+s-jf)/4.*z1*z2;
                ...
                double z1z2_upos=0.;
//                double Rk=0.;
//                std::cout<<this->GetPair(theDecay->GetPairNum())->IsUPOS()<<std::endl;

                if(this->GetPair(theDecay->GetPairNum())->GetPType()==0&&aa!=ir) { //&&this->GetPair(theDecay->GetPairNum())->IsUPOS() this->GetPair(theDecay->GetPairNum())->GetPType()!=10 || aa!=ir

//                  std::cout<<"Oops! "<<aa<<" "<<ir<<std::endl;                  
...
//                    std::cout<<AngCoeff::Racah(lOrder,sp,j2,l1p,sp2,j1)<<std::endl; 
		...
		if(fabs(z1z2)>1e-10||fabs(z1z2_upos)>1e-10) { //                 
		  ...
		  if(!KLGroupNum) {
		    ...
		    KLGroupNum=theDecay->IsKLGroup(NewKLGroup);
                  ...
                  if(aa==ir||this->GetPair(theDecay->GetPairNum())->GetPType()==10){   //this->GetPair(theDecay->GetPairNum())->IsUPOS()==false
//                    std::cout<<"capture"<<std::endl;
		    ...
		    if(!InterNum) {
		      ...
		      InterNum=theDecay->GetKLGroup(KLGroupNum)->IsInterference(NewInterference);
                  ...
                  else {//this->GetPair(theDecay->GetPairNum())->IsUPOS()==true
...
void CNuc::TransformOut(const Config& configure) {
  if(!(configure.paramMask & Config::USE_BRUNE_FORMALISM)) {
	...
	if(theLevel->IsInRMatrix()) {
	  ...
	  matrix_r tempGamma;
	  // this just initializes the values 
	  ...
	  //here the iterative procedure is done to determine the on-resonance parameters for each level for each j-pi
	  while(iteration<=maxIterations&&!done) {
	    ...
	    //calculation of the matrix C from Eq. 47 of Azuma et al. (2010)
	    matrix_r cMatrix;
